...
function resize_image_gd($orig_path, $new_path, $max_width, $max_height)
{
    ...
    // Calculate new size
    // Lines 37 and 39 are updated for PHP 8.1 and later. They use round() to prevent a warning that conversions from floats to ints are deprecated.
    if ($orig_width > $orig_height) {                     // If landscape
        $new_height = round($new_width / $orig_ratio);    // Set height using ratio
    } else {                                              // Otherwise
        $new_width  = round($new_height * $orig_ratio);   // Set width using ratio
    }
